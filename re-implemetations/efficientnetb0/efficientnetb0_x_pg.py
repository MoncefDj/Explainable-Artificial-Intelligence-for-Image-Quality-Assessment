...
class Config:
    def __init__(self, drive_base: str = DRIVE_BASE, dataset_base: str = DATASET_BASE_PATH):
        ...
        base_results_dir = os.path.join(drive_base, 'results_object_cxr_efficientnet_b0')
...
class ImageQualityModel(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.model_name = config.model_config.get('name', 'efficientnet_b0')
...
class GradCam:
    def __init__(self, model: nn.Module, model_args: DictAsMember, device: torch.device, input_size: Tuple[int, int]):
        ...
        if target_layer_name is None:
            if model_name == 'efficientnet_b0': target_layer_name = 'model.features.8'
            else: return
        try:
...
class NormGrad:
    def __init__(self, model: nn.Module, model_args: DictAsMember, input_size: Tuple[int, int]):
        ...
        if isinstance(target_layers_config, list) and all(isinstance(name, str) for name in target_layers_config):
            ...
            if model_name == 'efficientnet_b0': self.target_layers_names = ['model.features.8']
            else: return
...
class XAIVisualizer:
    def __init__(self, model_path: str, model_name: str, input_size: int, annotations_csv_path: Optional[str] = None):
        ...
        gradcam_target = 'model.features.8'
        normgrad_single_target = ['model.features.8']
        normgrad_combined_target = ['model.features.0', 'model.features.1', 'model.features.2', 'model.features.3',
